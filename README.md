...
Мультиарендный продукт: пациент общается с «Анной» в Telegram или в веб-виджете на
сайте клиники, бот квалифицирует обращение (услуга → срочность → имя → телефон
→ время) и мгновенно передаёт заявку живым сотрудникам в TG-группу клиники (Штаб).
Отдельный модуль ловит пропущенные звонки через Novofon и возвращает пациента
SMS-кой со ссылкой на бота. Владелец видит лиды и возвращённые рубли в веб-кабинете.

## Состав (ЭТАП A + B)

- **Витрина** (`web/`, React+Vite, `/`) — лендинг: калькулятор потерь, живой диалог
  Анны, тарифы, форма заявки (152-ФЗ), FAQ, `/privacy`. Дизайн из дизайн-пака
  (`web/src/design/`, read-only): доминирующий винный, золото акцентами, без градиентов.
- **Кабинет** (тот же проект, `/app`, PWA) — вход по magic-link через Telegram-бот,
  экраны «Сегодня» (лента лидов), «Деньги» (возвращённые ₽ + счётчик надёжности),
  «Настройки», онбординг-мастер.
- **Бэкенд** (`app/`, Python) — диалоговый движок (GigaChat + FSM), ручной режим
  записи (pending→подтверждение), cha-ching, редактирование ПДн, мультиарендность
  по `clinic_id` из JWT, лимиты, каскад каналов MAX→SMS, `api.py` для кабинета.

### Запуск всего одним скриптом

```bash
git clone <репозиторий> podhvat && cd podhvat
bash setup.sh          # 1-й запуск: ставит Docker, создаёт .env со случайным JWT_SECRET
nano .env              # впишите BOT_TOKEN, GIGACHAT_CREDENTIALS, DOMAIN, PUBLIC_BASE_URL, ...
bash setup.sh          # 2-й запуск: собирает витрину+кабинет и поднимает всё
curl -s http://localhost/health          # {"status":"ok","db":true,"version":"0.2.0"}
docker compose exec app uv run python -m scripts.seed_demo   # (опц.) демо-лиды
```

`docker compose` поднимает 3 сервиса: `app` (FastAPI+aiogram), `caddy` (собирает Vite-SPA
и раздаёт её + TLS Let's Encrypt + прокси на API), `backup` (ежедневный дамп SQLite в
`./backups`, 14 копий). Раз в неделю забирайте копию с VPS: `scp vps:~/podhvat/backups/*.gz .`.

### Фронтенд отдельно (разработка)

```bash
cd web && npm install && npm run dev     # http://localhost:5173
npm run build                             # прод-сборка (заявка → /api/public/lead-request)
```

### Тесты приёмки

```bash
uv run pytest -q        # 20 функциональных + 4 безопасности (+ базовые) — всё зелёное
```
