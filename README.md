# Подхват AI+ — ИИ-администратор для стоматологий

Проект состоит из двух частей в одном репозитории:

- **Сайт-витрина** — React + Vite (папка `src/`, деплой на Vercel). Продающий лендинг
  + политика ПДн `/privacy` + заглушка входа `/app`. Дизайн-система — в `src/design/`
  (read-only). См. раздел «Сайт-витрина» ниже.
- **Backend-ядро** — Python 3.12 (FastAPI + aiogram 3 + GigaChat + aiosqlite, папка `app/`,
  деплой на VPS в РФ). Мультиарендный бот: пациент общается с «Анной», бот квалифицирует
  обращение и передаёт заявку в TG-группу клиники; отдельный модуль ловит пропущенные
  звонки через Novofon. См. «Быстрый старт (backend)» ниже.

> Этап B (кабинет владельца, magic-link, каналы MAX/SMS.ru, приёмочные тесты) переносит
> `app/` → `backend/` и расширяет ядро — см. `docs/MMDV_ПРОМПТ_ПРИЛОЖЕНИЕ.md`.

---

## Сайт-витрина (React + Vite)
...
Все клиентские переменные — с префиксом `VITE_` (см. `.env.example`). Ни одна не обязательна
для сборки; пустые значения дают безопасные фолбэки.

| Переменная | Назначение |
|---|---|
| `VITE_SITE_DOMAIN` | Каноничный домен (по умолчанию `podhvatplus.ru`) |
| `VITE_TG_USERNAME` | Telegram основателя без `@` (кнопки «написать»). Пусто → плейсхолдер |
| `VITE_RUSTORE_URL` | Ссылка на приложение в RuStore. Пусто → плейсхолдер + QR от него |
| `VITE_YM_ID` | Счётчик Яндекс.Метрики. Пусто → цели логируются в консоль (`VITE_METRIKA_ID` — синоним) |
| `VITE_LEAD_WEBHOOK_URL` | Куда слать заявку. Пусто → serverless-функция `/api/lead` |
| `VITE_API_BASE` | База API бэкенда для кабинета `/app` (Этап B) |

Serverless-функция приёма заявки (`api/lead.js`, Node на Vercel) использует **серверные**
секреты (без префикса `VITE_`, в бандл не попадают):

| Переменная | Назначение |
|---|---|
| `TELEGRAM_BOT_TOKEN` | Токен бота-приёмщика заявок (@BotFather) |
| `TELEGRAM_CHAT_ID` | Chat ID, куда падают заявки с сайта |

Цели Метрики: `lead_submit`, `rustore_click`, `login_click`, `calc_used`.

### Деплой на Vercel
1. Импортируйте репозиторий в Vercel. Framework определится как **Vite** (см. `vercel.json`),
   команда сборки `npm run build`, output `dist/`. Папка `backend/`/`app/` на Vercel не собирается.
2. В **Settings → Environment Variables** добавьте нужные `VITE_*`, а для функции заявки —
   `TELEGRAM_BOT_TOKEN` и `TELEGRAM_CHAT_ID`.
3. Функция `api/lead.js` разворачивается автоматически как эндпоинт `POST /api/lead`
   (валидация полей + honeypot на сервере, отправка заявки владельцу в Telegram).
4. Привяжите домен `podhvatplus.ru`. SPA-роутинг (`/privacy`, `/app`) настроен через
   `rewrites` в `vercel.json`.
5. Проверьте Lighthouse (mobile) на превью-URL: Performance ≥ 90, Accessibility ≥ 95.

---

## Быстрый старт (backend): чистый Ubuntu VPS
...
### Установка на чистый Ubuntu 22 VPS (Timeweb/Beget, ~400–600 ₽/мес)
