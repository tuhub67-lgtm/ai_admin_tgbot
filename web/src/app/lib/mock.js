/* Мок-данные кабинета «Подхват AI+».
   Используются lib/api.js ТОЛЬКО при VITE_API_MOCK=1 (демо-превью). На реальном
   бэкенде не подключаются — данные идут из БД. Состояние мутируется в памяти
   сессии (смена статуса лида, сохранение настроек) — только для демо. */
