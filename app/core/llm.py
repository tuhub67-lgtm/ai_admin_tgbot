...
# ПДн (имя/телефон) в GigaChat НЕ передаём и НЕ просим модель их извлекать —
# их собирает детерминированно движок диалога и хранит только в локальной SQLite.
# Модель извлекает лишь неперсональные поля: услугу, срочность, удобное время.
SAVE_LEAD_FIELDS = ChatFunctionSpecification(
    name="save_lead_fields",
    description=(
        "Сохраняет неперсональные данные заявки по мере их появления в диалоге. "
        "Вызывай, когда пациент сообщил услугу, срочность или удобное время. "
        "Имя и телефон НЕ передавай — их фиксирует система отдельно."
    ),
    parameters={
        "type": "object",
        "properties": {
